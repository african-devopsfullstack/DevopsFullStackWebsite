...
def create_table(conn):
    cursor = conn.cursor()
    cursor.execute(
        "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT, fname VARCHAR(255) NOT NULL, lname VARCHAR(255) NOT NULL, \
...
def get_all_posts(conn):
    ...
    MAX(strftime('?', 'now') - strftime('?', posts.created_at)) AS time_elapsed_seconds,
...
def comments(
    conn, user_id, post_id, comment_text, likes, dislikes, parent_comment_id, created_at
):
    query = "INSERT INTO user_comments(user_id, post_id, comment_text, likes, dislikes, parent_comment_id, created_at) VALUES(?, ?, ?, ?, ?, ?, ?)"
    cursor = conn.cursor()
    cursor.execute(
        query,
        (
